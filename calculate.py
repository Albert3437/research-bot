...
class Calculation:
    def __init__(self, arch:str, arch_type:str, indicator_list:list, token:str, interval:str, year:int, bank:int=500):
        self.arch = arch
        self.type = arch_type
        self.indicator_list = indicator_list
        self.start_sum = bank
        self.df = pd.read_csv(f'data/{token}USDT/data_{year}/{interval}.csv')
    ...
    def calc_metric(self):
        ...
        if len(self.percent_list):
            ...
            for percent in self.percent_list:
                if percent >= 1:
                    profit_percent += 1
            metrics_dict['profitPercent'] = round(profit_percent, 5)
